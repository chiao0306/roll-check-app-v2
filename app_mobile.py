...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    *   **加總範圍 (預設)**：
        *   **機ROLL 車修** = Sum(本體未再生 + 本體再生 + 軸頸未再生 + 軸頸再生)
        *   **機ROLL 銲補** = Sum(本體銲補 + 軸頸銲補)
        *   **機ROLL 拆裝** = Sum(新品組裝 + 舊品拆裝)
    *   **例外過濾 (特規介入)**：
    ...
    *   **適用類型**：數值超規、尺寸異常、物理流程錯誤。
    *   **表格欄位定義**：
        - `id`: "滾輪編號" (例如：#1)
        - `val`: "實測數值" (例如：295.05)
        - `target`: "規格標準" (例如：294.00 - 295.00)
        - `calc`: "判定結果/原因" (例如：超規 0.05 或 流程倒置)
